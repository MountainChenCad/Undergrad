...
\SetKw{KwRepeat}{repeat}
\SetKw{KwUntil}{until}
\SetKw{KwFrozen}{Frozen}
\SetKw{KwTrainable}{Trainable}
...
\KwInit{随机初始化适配器参数 $\theta_{Ad}$}
\KwFrozen{VLM参数 $\theta_V, \theta_T$}
\KwTrainable{适配器参数 $\theta_{Ad}$}
\For{$epoch = 1$ to $E$}{
    \For{each batch $\{(x_{H,i}, y_i=c)\}$ from $\mathcal{D}_{\text{base}}$}{
        ...
        $z_{T,i} \leftarrow z_{T,c}$ (retrieve corresponding semantic feature)\;
...
\KwFrozen{参数 $\theta_{Ad}^*, \theta_V, \theta_T$, (可选) $\theta_F$}
