...
\While{not converged}{
  ...
  \For{each meta-task set $\mathbf{T}_i$}{
    ...
    \For{each task $\mathcal{T}_{\mathcal{S},j} = (S_{\mathcal{S},j}, Q_{\mathcal{S},j})$ in $\mathcal{S}_{\mathcal{T},i}$}{
       ...
       Compute loss $\mathcal{L}_j = \mathcal{L}(f_{\Theta}(\mathcal{T}_{\mathcal{S},j}), Y_j)$ using Eq. (4.3)\; % 引用本文中的公式编号
       % 执行一步或多步梯度更新来得到 Theta'_i，这里简化为基于总损失更新
    }
    Compute total support loss $\mathcal{L}_{support} = \sum_{j=1}^C \mathcal{L}_j$\;
    $\Theta'_i \leftarrow \Theta - \alpha \nabla_{\Theta} \mathcal{L}_{support}$ \Comment*[r]{内循环参数适应}

    \Comment{外循环：计算元梯度}
    Initialize query loss $\mathcal{L}_{query\_total} = 0$\;
  ...
  $\theta \leftarrow \theta - \beta \cdot (\nabla_\theta \mathcal{L}_{meta} / B)$ \Comment*[r]{元更新 (B为批大小)}
...
\For{each query sample $\overline{I}_q$ in $Q_{new}$}{
  Compute output probabilities $P(Y_* = y_N | \mathcal{T}_{new})$ using $f_{\Theta^*}(G_{\mathcal{T}_{new}})$\; % 直接使用元参数进行预测
