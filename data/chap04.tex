...
首先，为有效捕捉HRRP序列内部的结构信息并可能增强对角度变化的鲁棒性，我们采用Gramian Angular Field (GAF) 技术\upcite{liu_few-shot_2021}将一维的HRRP序列 $\mathbf{x} \in \mathbb{R}^L$ 转换为二维的GAF图像 $I \in \mathbb{R}^{L \times L}$。该转换过程包含三个主要步骤：
...
本文不采用预定义的、基于角度差的固定边，而是借鉴GNN for FSL\upcite{garcia_gnn_2018}的思想，让模型通过学习来动态地确定样本间的关系强度。首先，使用一个2D-CNN来提取每个GAF图像 $I_i$ 的初始特征嵌入 $\mathbf{e}_i = \phi(I_i) \in \mathbb{R}^d$。为了在图中融入类别信息并初始化查询节点的标签估计，我们构建初始节点表示 $\mathbf{h}_i^{(0)}$。对于支持集中的样本 $i$（其GAF图像为 $I_i^s$，标签为 $l_i$），其初始节点表示由其特征嵌入 $\mathbf{e}_i^s = \phi(I_i^s)$ 与其类别标签的独热编码向量 $\mathbf{o}(l_i) \in \{0, 1\}^N$ 拼接而成：
\begin{equation}
...
\begin{table*}[h!]
\caption{所提GAF-MLGNN及对比不同方法在标准小样本场景下的性能}
